    ...
    for(int i=1; i<imax-1; i++)
        {
            ...
            {
                double dpdx = (-half*u(i-1,j,0)+zero*u(1,j,0)+half*u(i+1,j,0))/dx;        //First derivative of pressure w.r.t. x
                double dudx = (-half*u(i-1,j,1)+zero*u(1,j,1)+half*u(i+1,j,1))/dx;        //First derivative of x velocity w.r.t. x
                double dvdx = (-half*u(i-1,j,2)+zero*u(1,j,2)+half*u(i+1,j,2))/dx;        //First derivative of y velocity w.r.t. x
    ...
    for(int i=imax-1; i>1; i--)
        {
            for(int j=jmax-1; j>1; j--)
            {
                ...
                u(i,j,1)=u(i,j,1)-(dt(i,j)/rho)*( rho*(u(i,j,1)*dudx + u(i,j,2)*dudy) + dpdx - rmu*(d2udx2 + d2udy2)
                             - s(i,j,1) );
                u(i,j,2)=u(i,j,2)-(dt(i,j)/rho)*( rho*(u(i,j,2)*dvdx + u(i,j,1)*dvdy) + dpdy - rmu*(d2vdx2 + d2vdy2)
                             - s(i,j,2) );
        }
    }
    /* Symmetric Gauss-Siedel: Backward Sweep  */
    ...
    for(int i=1; i<imax-1; i++)
        {
            for(int j=1; j<jmax-1; j++)
            {
                double dpdx = (-half*uold(i-1,j,0)+zero*uold(1,j,0)+half*uold(i+1,j,0))/dx;        //First derivative of pressure w.r.t. x
                double dudx = (-half*uold(i-1,j,1)+zero*uold(1,j,1)+half*uold(i+1,j,1))/dx;        //First derivative of x velocity w.r.t. x
                double dvdx = (-half*uold(i-1,j,2)+zero*uold(1,j,2)+half*uold(i+1,j,2))/dx;        //First derivative of y velocity w.r.t. x
                ...
                u(i,j,1)=uold(i,j,1)-(dt(i,j)/rho)*( rho*(uold(i,j,1)*dudx + uold(i,j,2)*dudy) + dpdx - rmu*(d2udx2 + d2udy2)
                             - s(i,j,1) );
                u(i,j,2)=uold(i,j,2)-(dt(i,j)/rho)*( rho*(uold(i,j,2)*dvdx + uold(i,j,1)*dvdy) + dpdy - rmu*(d2vdx2 + d2vdy2)
                             - s(i,j,2) );
        }
    }
