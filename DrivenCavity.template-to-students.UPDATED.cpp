...
#define imax 81     /* Number of points in the x-direction (use odd numbers only) */
#define jmax 81     /* Number of points in the y-direction (use odd numbers only) */
...
/*--------- User sets inputs here  --------*/
  ...
  const double cfl  = 0.9;              /* CFL number used to determine time step, reduced to .1 for now */
  ...
  const double fsmall = 1.e-14;         /* small parameter modified to be e-14 instead of -20*/
  ...
  */
  ...
  /* Set up output files (history and solution)  */    
    ...
    if (imms==0)
    {
        if (isgs==0)
        {
            filenameStream << "./history_pj_"<< imax << "_Re="<< Re << "_kappa=" << rkappa << ".dat";
        }
        else
        {
            filenameStream << "./history_sgs_"<< imax << "_Re="<< Re << "_kappa=" << rkappa << ".dat";
        }
    }
    else
    {
        if (isgs==0)
        {
            filenameStream << "./history_pj_"<< imax << "_Re="<< Re << "_C4=" << Cx << "_mms.dat";
        }
        else
        {
            filenameStream << "./history_sgs_"<< imax << "_Re="<< Re << "_C4=" << Cx << "_mms.dat";
        }  
    }
    ...
    filenameStream.clear(); // Clear any error flags

    if (imms==0)
    {
        if (isgs==0)
        {
            filenameStream << "./cavity_pj_"<< imax << "_Re="<< Re << "_kappa=" << rkappa << ".dat";
        }
        else
        {
            filenameStream << "./cavity_sgs_"<< imax << "_Re="<< Re << "_kappa=" << rkappa << ".dat";
        }
    }
    else
    {
        if (isgs==0)
        {
            filenameStream << "./cavity_pj_"<< imax << "_Re="<< Re << "_C4=" << Cx << "_mms.dat";
        }
        else
        {
            filenameStream << "./cavity_sgs_"<< imax << "_Re="<< Re << "_C4=" << Cx << "_mms.dat";
        }  
    }
...
void bndry( Array3& u )
{
    ...
    */
    ...
    // BC: No slip walls on all sides except top lid
    /* Side Walls */
    for(int j = 0; j<jmax; j++)
    {
        ...
        u(i,j,0) = two*u(1,j,0) - u(2,j,0);    /* 2nd Order BC pressure */
...
void compute_time_step( Array3& u, Array2& dt, double& dtmin )
{
    ...
    */
    ...
    dtmin = 1.0e+99;          //Initialize minimum time step to large number

    double nu=rmu/rho;          //Kinematic viscosity
    double dtvisc=fourth*dx*dy/nu;  //Viscous time step stability criteria (constant over domain) AKA diffusive time step
    double uinf2=uinf*uinf;         //Freestream velocity squared

    for(i=1; i<imax-1; i++)
    {
        for(j=1; j<jmax-1; j++)
        {
            double ui=u(i,j,1);          //Local x velocity
            double vi=u(i,j,2);          //Local y velocity

            double uvel2=ui*ui+vi*vi;    //Local velocity squared
            double beta2=std::max(uvel2,rkappa*uinf2);                       //Beta squared parameter for time derivative preconditioning
            double lambda_x=half*(std::abs(ui)+sqrt(ui*ui+four*beta2));      //Max absolute value eigenvalue in (x,t)
            double lambda_y=half*(std::abs(vi)+sqrt(vi*vi+four*beta2));      //Max absolute value eigenvalue in (y,t)
            double lambda_max=std::max(lambda_x,lambda_y);      //Max absolute value eigenvalue (used in convective time step computation)
            if(lambda_max<fsmall)
            {
                lambda_max=fsmall;
            }
            double dtconv=std::min(dx,dy)/lambda_max;          //Local convective time step restriction
            
            dt(i,j) = cfl*std::min(dtconv,dtvisc);   //Local time step based on min of convective and viscous criteria
            dtmin=std::min(dt(i,j), dtmin);   //Find minimum time step over entire domain
        }
    }

    for(i=0; i<imax; i++)  //Set boundary time steps to minimum interior time step
    {
        for(j=0; j<jmax; j++)
        {
            dt(i,j)=std::max(dtmin,fsmall);
        }
    }
    // CODE ADDED
}  
...
void Compute_Artificial_Viscosity( Array3& u, Array2& viscx, Array2& viscy )
{
    ...
    */
    ...
    double lambda_xmax=0.0;      //Max absolute value e-value in (x,t) over entire domain
    double lambda_ymax=0.0;      //Max absolute value e-value in (
    ...
    for(i=2; i<imax-2; i++)
    {
        for(j=2; j<jmax-2; j++)
        {
            ...
            double lambda_x=half*(std::abs(ui)+sqrt(ui*ui+four*beta2));      //Max absolute value eigenvalue in (x,t)
            double lambda_y=half*(std::abs(vi)+sqrt(vi*vi+four*beta2));      //Max absolute value eigenvalue in (y,t)
            lambda_xmax=std::max(lambda_x,lambda_xmax);      //Max absolute value eigenvalue
            lambda_ymax=std::max(lambda_y,lambda_ymax);      //Max absolute value eigenvalue
            
        }
    }

    for(i=2; i<imax-2; i++)  //Set boundary artificial viscosities to zero
    {
        for(j=2; j<jmax-2; j++)
        {
            double ui=u(i,j,1);          //Local x velocity
            ...
            double uvel2=ui*ui+vi*vi;    //Local velocity squared
            double beta2=std::max(uvel2,rkappa*uinf2);
            ...
            double d4pdx4=(u(i-2,j,0) - 4.0*u(i-1,j,0) + 6.0*u(i,j,0) - 4.0*u(i+1,j,0) + u(i+2,j,0))/dx4;      //4th derivative of pressure w.r.t. x
            double d4pdy4=(u(i,j-2,0) - 4.0*u(i,j-1,0) + 6.0*u(i,j,0) - 4.0*u(i,j+1,0) + u(i,j+2,0))/dy4;      //4th derivative of pressure w.r.t. y
            ...
            viscx(i,j) =-std::abs(lambda_xmax)*Cx*dx3*d4pdx4/beta2;
            viscy(i,j) =-std::abs(lambda_ymax)*Cy*dy3*d4pdy4/beta2;
        }
    }
    ...
    for(int i=1; i<imax-1; i++)
        {
            ...
            {
                double dpdx = (-half*u(i-1,j,0)+zero*u(i,j,0)+half*u(i+1,j,0))/dx;        //First derivative of pressure w.r.t. x
                double dudx = (-half*u(i-1,j,1)+zero*u(i,j,1)+half*u(i+1,j,1))/dx;        //First derivative of x velocity w.r.t. x
                double dvdx = (-half*u(i-1,j,2)+zero*u(i,j,2)+half*u(i+1,j,2))/dx;        //First derivative of y velocity w.r.t. x
                double dpdy = (-half*u(i,j-1,0)+zero*u(i,j,0)+half*u(i,j+1,0))/dy;        //First derivative of pressure w.r.t. y
                double dudy = (-half*u(i,j-1,1)+zero*u(i,j,1)+half*u(i,j+1,1))/dy;        //First derivative of x velocity w.r.t. y
                double dvdy = (-half*u(i,j-1,2)+zero*u(i,j,2)+half*u(i,j+1,2))/dy;        //First derivative of y velocity w.r.t. y
                
                double d2udx2 = (u(i-1,j,1)-two*u(i,j,1)+u(i+1,j,1))/dx2;      //Second derivative of x velocity w.r.t. x
                double d2vdx2 = (u(i-1,j,2)-two*u(i,j,2)+u(i+1,j,2))/dx2;      //Second derivative of y velocity w.r.t. x
                double d2udy2 = (u(i,j-1,1)-two*u(i,j,1)+u(i,j+1,1))/dy2;      //Second derivative of x velocity w.r.t. y
                double d2vdy2 = (u(i,j-1,2)-two*u(i,j,2)+u(i,j+1,2))/dy2;      //Second derivative of y velocity w.r.t. y
                
                double uvel2 = u(i,j,1)*u(i,j,1)+u(i,j,2)*u(i,j,2);       //Velocity squared at node
                double beta2 = std::max(uvel2,rkappa*uinf2);       //Beta squared parameter for time derivative preconditioning 

                u(i,j,0)=u(i,j,0)-dt(i,j)*beta2*( rho*(dudx + dvdy) - (viscx(i,j)+viscy(i,j)) - s(i,j,0));
                u(i,j,1)=u(i,j,1)-(dt(i,j)/rho)*( rho*(u(i,j,1)*dudx + u(i,j,2)*dudy) + dpdx - rmu*(d2udx2 + d2udy2)
                             - s(i,j,1) );
                u(i,j,2)=u(i,j,2)-(dt(i,j)/rho)*( rho*(u(i,j,1)*dvdx + u(i,j,2)*dvdy) + dpdy - rmu*(d2vdx2 + d2vdy2)
                             - s(i,j,2) );
        }
    }
...
void point_Jacobi( Array3& u, Array3& uold, Array2& viscx, Array2& viscy, Array2& dt, Array3& s )
{
    ...
    */
    double  dx2 = dx*dx;
    double  dy2 = dy*dy;
    double uinf2 = uinf*uinf;

    for(int i=1; i<imax-1; i++)
    {
            for(int j=1; j<jmax-1; j++)
            {
                double dpdx = (-half*uold(i-1,j,0)+zero*uold(i,j,0)+half*uold(i+1,j,0))/dx;        //First derivative of pressure w.r.t. x
                double dudx = (-half*uold(i-1,j,1)+zero*uold(i,j,1)+half*uold(i+1,j,1))/dx;        //First derivative of x velocity w.r.t. x
                double dvdx = (-half*uold(i-1,j,2)+zero*uold(i,j,2)+half*uold(i+1,j,2))/dx;        //First derivative of y velocity w.r.t. x
                double dpdy = (-half*uold(i,j-1,0)+zero*uold(i,j,0)+half*uold(i,j+1,0))/dy;        //First derivative of pressure w.r.t. y
                double dudy = (-half*uold(i,j-1,1)+zero*uold(i,j,1)+half*uold(i,j+1,1))/dy;        //First derivative of x velocity w.r.t. y
                double dvdy = (-half*uold(i,j-1,2)+zero*uold(i,j,2)+half*uold(i,j+1,2))/dy;        //First derivative of y velocity w.r.t. y
                
                double d2udx2 = (uold(i-1,j,1)-two*uold(i,j,1)+uold(i+1,j,1))/dx2;      //Second derivative of x velocity w.r.t. x
                double d2vdx2 = (uold(i-1,j,2)-two*uold(i,j,2)+uold(i+1,j,2))/dx2;      //Second derivative of y velocity w.r.t. x
                double d2udy2 = (uold(i,j-1,1)-two*uold(i,j,1)+uold(i,j+1,1))/dy2;      //Second derivative of x velocity w.r.t. y
                double d2vdy2 = (uold(i,j-1,2)-two*uold(i,j,2)+uold(i,j+1,2))/dy2;      //Second derivative of y velocity w.r.t. y
                
                double uvel2 = uold(i,j,1)*uold(i,j,1)+uold(i,j,2)*uold(i,j,2);       //Velocity squared at node
                double beta2 = std::max(uvel2,rkappa*uinf2);       //Beta squared parameter for time derivative preconditioning 

                u(i,j,0)=uold(i,j,0)-dt(i,j)*beta2*( rho*(dudx + dvdy) - (viscx(i,j)+viscy(i,j)) - s(i,j,0));
                u(i,j,1)=uold(i,j,1)-(dt(i,j)/rho)*( rho*(uold(i,j,1)*dudx + uold(i,j,2)*dudy) + dpdx - rmu*(d2udx2 + d2udy2)
                             - s(i,j,1) );
                u(i,j,2)=uold(i,j,2)-(dt(i,j)/rho)*( rho*(uold(i,j,1)*dvdx + uold(i,j,2)*dvdy) + dpdy - rmu*(d2vdx2 + d2vdy2)
                             - s(i,j,2) );
            }
    }
  ...
  */
  ...
  double beta2;
    ...
    for(i=1; i<imax-1; i++)
    {
        ...
        {
            double uvel2 = uold(i,j,1)*uold(i,j,1)+uold(i,j,2)*uold(i,j,2);       //Velocity squared at node
            beta2 = std::max(uvel2,rkappa*uinf2);

            dpdt = (u(i,j,0)-uold(i,j,0))/(dt(i,j)*beta2);   //Time derivative of variable at node
            res[0] += dpdt*dpdt;   // L2 norm sum
            dudt = rho*(u(i,j,1)-uold(i,j,1))/dt(i,j);   //Time derivative of variable at node
            res[1] += dudt*dudt;   // L2 norm sum of time derivative over domain
            dvdt = rho*(u(i,j,2)-uold(i,j,2))/dt(i,j);   //Time derivative of variable at node
            res[2] += dvdt*dvdt;   // L2 norm sum
        }
    }

    for(k=0; k<neq; k++)
    {
        res[k] = sqrt( res[k]/(double)((imax-2)*(jmax-2)) );   /* RMS residual over entire domain */
    }

    if(n<=2)  /* Store initial residuals from first iteration, changed to less than two since it kept blowing up after the first */
    {
        for(k=0; k<neq; k++)
        {
            if(res[k]<fsmall)
            {
                res[k] = fsmall;   /* Prevent division by zero in normalization */
            }
            resinit[k] = res[k];
        }
    }
    ...
    for(k=0; k<neq; k++)
    {
        res[k] = res[k]/resinit[k];   /* Normalize residuals by initial residuals */
    }

    conv = res[0];   /* Initialize conv to continuity residual */
    for(k=1; k<neq; k++)
    {
        conv = std::min(conv, res[k]);   /* Find minimum residual over all equations */
    }  
...
void Discretization_Error_Norms( Array3& u ) 
{
    ...
    */
    ...
    if(imms==1)
    {
        ...
        for(int i=0; i<imax; i++)
        {
            x = xmin + (xmax - xmin)*(double)(i)/(double)(imax - 1);
            
            for(int j=0; j<jmax; j++)
            {
                y = ymin + (ymax - ymin)*(double)(j)/(double)(jmax - 1);
                for(int k=0; k<neq; k++)
                {
                    DE = fabs( u(i,j,k) - umms(x,y,k) );   /* Discretization error at node */

                    rL1norm[k]   += DE;
                    rL2norm[k]   += DE*DE;
                    rLinfnorm[k]  = std::max( rLinfnorm[k], DE );
                }
            }
        }
        ...
        for(int k=0; k<neq; k++)
        {
            rL1norm[k]   = rL1norm[k]/(double)((imax-2)*(jmax-2));
            rL2norm[k]   = sqrt( rL2norm[k]/(double)((imax-2)*(jmax-2)) );
        }

        /* Output discretization error norms */
