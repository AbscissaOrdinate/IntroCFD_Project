  ...
  int k;                       /* k index (# of equations) */
    ...
    for(i=1; i<imax-1; i++)
    {
        for(j=1; j<jmax-1; j++)
        {
            for(k=0; k<neq; k++)
            {
                double dudt = (u(i,j,k)-uold(i,j,k))/dt(i,j);   //Time derivative of variable at node
                res[k] += dudt*dudt;   // L2 norm sum of time derivative over domain
            }
        }
    }
    ...
    if(n==ninit)  /* Store initial residuals from first iteration */
    {
        for(k=0; k<neq; k++)
        {
            if(res[k]<fsmall)
